...
This README file corresponds to the datafiles that are present in the folder "Info_regarding_FLUIDIGM_samples_and_data".
...
This folder contains info regarding the samples that were selected for the FLUIDIGM analyses. It additionally contains processed data files in Excel files, with in each Excel sheet the multiple steps of manual data processing (e.g. removing data columns of faulty primer pairs, normalizatoin using house-keeping genes, determination of primer pair efficiency, etc.). The last sheet in each file contains the hypothesis testings that were performed to get the p-values.
